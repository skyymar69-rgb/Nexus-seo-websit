  ...
  return (
    <div className="space-y-6">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-3 py-3 rounded-xl border border-white/10 bg-white/[0.03] text-white text-sm font-medium hover:bg-white/[0.06] transition-colors"
      >
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-white/30">ou avec votre e-mail</span>
        <div className="flex-1 h-px bg-white/10" />
      </div>

      {error && (
        <div role="alert" aria-live="assertive" className="p-3 rounded-xl bg-rose-500/10 border border-rose-500/20">
        ...
        <div>
          ...
          {password && (
            <div className="mt-2 flex items-center gap-2" id="password-strength" role="meter" aria-valuenow={strength.score} aria-valuemin={0} aria-valuemax={4} aria-label={`Force du mot de passe : ${strength.label || 'non évaluée'}`}>
              <div className="flex-1 h-1 bg-white/5 rounded-full overflow-hidden" aria-hidden="true">
                <div className={`h-full transition-all ${strength.color}`} style={{ width: `${(strength.score / 4) * 100}%` }} />
              </div>
              <span className="text-xs text-white/40" aria-live="polite">{strength.label}</span>
            </div>
          )}
        </div>

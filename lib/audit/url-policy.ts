/**
 * Politique d'URL du crawler : garde SSRF et résolution de l'URL de départ.
 * Portage d'OpenSEO (MIT).
 *
 * Bloque les schémas non http(s), les adresses privées et de bouclage, les
 * hôtes internes, et vérifie par DNS-over-HTTPS que l'hôte de départ ne
 * résout pas vers une adresse privée. Les liens découverts en cours de crawl
 * n'ont qu'une vérification synchrone (pas de DNS par lien).
 */
...
const BLOCKED_HOSTS = new Set(['localhost', 'metadata.google.internal', 'metadata', '169.254.169.254', '100.100.100.200'])
...
async function resolveAddressRecords(hostname: string, type: 'A' | 'AAAA'): Promise<string[]> {
  ...
  return body.Answer.filter((answer) => answer.type === expectedType && typeof answer.data === 'string').map(
    (answer) => normalizeHost(answer.data as string),
  )
}
...
async function hostnameResolvesToBlockedAddress(hostname: string): Promise<boolean> {
  ...
  try {
    const [v4, v6] = await Promise.all([resolveAddressRecords(host, 'A'), resolveAddressRecords(host, 'AAAA')])
    const addresses = [...v4, ...v6]
    if (addresses.length === 0) return false
    return addresses.some((address) => isPrivateIpv4(address) || isPrivateIpv6(address))
  } catch {
    return false
  }
}

/** Vérification synchrone pour les URL découvertes en cours de crawl. */
...
export async function normalizeAndValidateStartUrl(input: string): Promise<string> {
  let raw = input.trim()
  if (!raw) throw new CrawlTargetError('INVALID_URL', 'URL requise')
  if (!raw.startsWith('http://') && !raw.startsWith('https://')) raw = `https://${raw}`
  let parsed: URL
  try {
    parsed = new URL(raw)
  } catch {
    throw new CrawlTargetError('INVALID_URL', 'URL invalide')
  }
  if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
    throw new CrawlTargetError('INVALID_URL', 'Seuls http et https sont pris en charge')
  }
  if (isBlockedHost(parsed.hostname)) {
    throw new CrawlTargetError('TARGET_BLOCKED', 'Cette adresse ne peut pas être auditée')
  }
...
/**
 * Suit les redirections de l'URL de départ pour ancrer l'audit sur l'origine
 * réelle du site (apex vers www, .net vers .com). Chaque saut repasse la
 * validation complète, une redirection ne peut donc pas emmener l'audit vers
 * une cible interdite.
 */
export async function resolveStartUrlRedirects(startUrl: string, userAgent: string): Promise<string> {

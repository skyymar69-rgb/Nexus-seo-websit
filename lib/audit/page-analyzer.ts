/**
 * Analyseur de page HTML sur le tokenizer en flux d'htmlparser2.
 * Portage d'OpenSEO (MIT).
 *
 * Délibérément pas un parseur DOM : cheerio construit un arbre de 5 à 10 fois
 * la taille du HTML par page ; avec une dizaine d'analyses concurrentes c'est
 * la première cause de dépassement mémoire d'un crawler. Le tokenizer ne
 * garde que le texte accumulé et les champs extraits.
 */
...
// Plafonds par page : les pièges à robots et méga-menus portent des milliers
// de liens par page.
...
export function analyzeHtml(
  ...
  const handleLinkTag = (attribs: Record<string, string>) => {
    ...
    if (rel === 'canonical') canonical ??= attribs['href'] ?? null
    else if (rel === 'alternate' && attribs['hreflang']) hreflangTags.push(attribs['hreflang'])
  }
      ...
      switch (name) {
        ...
        case 'img':
          if (images.length < MAX_EXTRACTED_IMAGES) {
            images.push({ src: attribs['src'] ?? null, alt: 'alt' in attribs ? attribs['alt'] : null })
          }
          break
        case 'script':
          ...
          break
        case 'a': {
          // HTML interdit les <a> imbriqués : les navigateurs ferment l'ouvert.
          closeAnchor()
